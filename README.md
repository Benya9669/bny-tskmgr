...
- устанавливаемая PWA-оболочка с standalone-режимом и адаптивными иконками;
- сохранённая локальная копия данных для чтения при временном отсутствии сети; офлайн-изменения появятся вместе с очередью синхронизации в `0.2.0`;
- очередь офлайн-изменений задач: создание, редактирование и удаление отправляются пакетно после восстановления сети и защищены client mutation ID от повторного применения;
- при конфликте офлайн-изменения задачи пользователь явно выбирает серверную версию или повторно применяет локальную к актуальной версии задачи;
- короткоживущий access token и refresh token с ротацией; в настройках аккаунта можно просмотреть и завершить активные сессии устройств;
- часовой пояс профиля в формате IANA для совместимой интерпретации сроков веб- и мобильным клиентами;
- `/api/v1/sync` возвращает bounded page до 500 изменений; продолжение выполняется opaque `next_cursor` в рамках фиксированного `snapshot`, а ответы от 1 KiB сжимаются gzip при `Accept-Encoding: gzip`;
...
| `TASKFLOW_PASSWORD_RESET_TTL_HOURS` | `1` | Срок действия одноразовой ссылки для сброса пароля |
| `TASKFLOW_EMAIL_CHANGE_TTL_HOURS` | `24` | Срок действия подтверждения нового email |
...
| `GET/POST` | `/api/v1/kanban/columns` | Список и создание пользовательских колонок |
| `PATCH/DELETE` | `/api/v1/kanban/columns/{id}` | Изменение или удаление колонки с переносом задач |
| `POST` | `/api/v1/kanban/columns/reorder` | Атомарное изменение порядка колонок |
| `GET/POST` | `/api/v1/tasks` | Список и создание задач |
| `PATCH/DELETE` | `/api/v1/tasks/{id}` | Изменение и мягкое удаление |
| `POST` | `/api/v1/tasks/{id}/move` | Атомарное перемещение и ручной порядок карточек канбана |
...
| `GET` | `/api/v1/messages?task_id={id}` | Активные сообщения обсуждения задачи |
| `POST` | `/api/v1/tasks/{id}/messages` | Добавление сообщения в обсуждение |
| `PATCH/DELETE` | `/api/v1/messages/{id}` | Редактирование и мягкое удаление сообщения |
| `GET/POST` | `/api/v1/note-folders` | Список и создание папок заметок |
| `PATCH/DELETE` | `/api/v1/note-folders/{id}` | Переименование и мягкое удаление папки без удаления заметок |
| `GET/POST` | `/api/v1/notes` | Список и создание Markdown-заметок |
| `PATCH/DELETE` | `/api/v1/notes/{id}` | Автосохранение и мягкое удаление заметки |
| `GET/POST` | `/api/v1/note-links` | Список и создание связей заметок с задачами и проектами |
| `DELETE` | `/api/v1/note-links/{id}` | Мягкое удаление связи заметки |
...
| `POST` | `/api/v1/data/import/yougile` | Атомарный импорт JSON-экспорта YouGile |
...
Экспорт из окна «Экспорт и импорт» содержит активные и архивные проекты, пользовательские канбан-колонки, ручной порядок карточек, задачи, элементы чек-листов, обсуждения, Markdown-заметки с папками и их связи с задачами и проектами. Email, пароль, bearer-токены и другие данные аккаунта в файл не включаются. Формат версии 6 сохраняет все эти сущности, при этом старые экспорты версий 1–5 продолжают импортироваться. Импорт ограничен 1 МБ, полностью проверяется до записи и добавляет сущности с новыми UUID. Существующие данные не изменяются; при любой ошибке импорт целиком откатывается.

В том же окне можно выбрать JSON-экспорт проекта YouGile. Корневой проект, колонки, карточки, дерево подзадач, совместимые комментарии и системные события переносятся в TaskFlow; повторяющиеся по названию и системному смыслу колонки переиспользуются. История дочерней подзадачи прикрепляется к обсуждению корневой карточки с понятным префиксом. Исходные доска и колонка указываются в описании задачи. Исполнители, стикеры, описания дочерних подзадач и неизвестные типы системных событий перечисляются в отчёте как пропущенные. Некорректные ссылки, циклы и задачи вне досок приводят к полному откату импорта.

Раздел «Заметки» поддерживает заголовки, списки, цитаты, выделение, inline-код, блоки кода, безопасные HTTP(S)-ссылки и внутренние ссылки `[[Название заметки]]`. Уникальная внутренняя ссылка открывает заметку, а отсутствующая или неоднозначная остаётся помеченной без случайного перехода. Поиск работает по заголовку и Markdown-содержимому. В блоке «Связано с» заметку можно привязать к задачам и проектам. Название, Markdown-текст и выбранная папка автоматически сохраняются после короткой паузы; индикатор над редактором показывает текущее состояние. Удаление папки не удаляет её заметки — они переходят в «Без папки».

На доске карточки перетаскиваются между пользовательскими колонками и вручную упорядочиваются внутри них. Представление «Неделя» показывает семь локальных календарных дней и задачи без даты; drag-and-drop назначает день, а перенос в «Без даты» очищает планирование. Срок выполнения остаётся независимым. На touch-устройствах и с клавиатуры доступна кнопка переноса карточки.

Задача может повторяться каждый день, неделю или месяц. После её выполнения TaskFlow атомарно создаёт один следующий экземпляр в колонке «Запланировано» с датой относительно дня выполнения. Проект, описание, приоритет, оценка и правило повторения сохраняются; срок выполнения не копируется, чтобы не переносить устаревший дедлайн.

Для задач со сроком можно включить браузерные напоминания в срок, за 15 минут, час и день. Разрешение запрашивается при сохранении такой задачи. Напоминания работают, пока TaskFlow открыт в браузере.

Добавляйте к задачам до 20 тегов через запятую и используйте тег в панели фильтров. Набор условий фильтрации можно сохранить под именем: он хранится в настройках текущего браузера.

Укажите оценку задачи в минутах, чтобы видеть нагрузку по дням в недельном календаре. Доступное время на день настраивается там же и хранится в текущем браузере; дни с превышением выделяются как перегруженные.

Кнопка `Фокус` запускает локальный таймер для одной активной задачи, сохраняемый при перезагрузке страницы. При старте задача переводится в работу, а в карточке задачи доступна неизменяемая история создания, изменений и перемещений.
...
1. Room хранит задачи, проекты, канбан-колонки, элементы чек-листов, сообщения обсуждений, папки заметок, заметки и их связи локально.
...
5. Удалённые записи приходят из `/sync` с `deleted_at`, поэтому удаление воспроизводится на всех устройствах. В ответе синхронизации колонки находятся в `kanban_columns`, элементы подзадач — в `checklist_items`, обсуждения — в `task_messages`, папки заметок — в `note_folders`, заметки — в `notes`, связи — в `note_links`.
